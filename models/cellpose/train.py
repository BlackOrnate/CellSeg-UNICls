...
class FocalTverskyLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # flatten
        input = torch.softmax(input, dim=-1).view(B, C, -1)
        target = target.view(B, C, -1)

        # calculate true positives, false positives and false negatives
        tp = (input * target).sum(dim=2)
        fp = ((1 - target) * input).sum(dim=2)
        fn = (target * (1 - input)).sum(dim=2)

        Tversky = (tp + self.smooth) / (tp + self.alpha_t * fn + self.beta_t * fp + self.smooth)
        FocalTversky = (1 - Tversky) ** self.gamma_f

        return FocalTversky[:,1:].mean()
...
def _loss_fn_class(lbl, y, class_weights=None):
    ...
    #
    # criterion4 = FocalTverskyLoss()
    # loss4 = criterion4(y[:, :-3], lbl[:, 0].long())
    #
    # loss = loss3 + loss4
    ...
    ce = criterion3(y[:, :-3], target)

    ce = ce * mask
    loss3 = ce.sum() / mask.sum()

    # FocalTversky
    criterion4 = FocalTverskyLoss()
    loss4 = criterion4(y[:, :-3], target)
...
def _process_train_test(train_data=None, train_labels=None, train_type_maps=None, train_files=None,
                        train_labels_files=None, train_probs=None,
                        test_data=None, test_labels=None, test_type_maps=None, test_files=None,
                        test_labels_files=None, test_probs=None,
                        load_files=True, min_train_masks=5, compute_flows=False, normalize_params={"normalize": False},
    ...
    if train_labels is not None:
        train_labels = dynamics.labels_to_flows(train_labels, type_maps=train_type_maps, files=train_files,
                                                device=device)
        if test_labels is not None:
            test_labels = dynamics.labels_to_flows(test_labels, type_maps=test_type_maps, files=test_files,
                                                   device=device)
    elif compute_flows:
...
def train_seg(net,
              train_data=None, train_labels=None, train_type_maps=None, train_files=None,
              train_labels_files=None, train_probs=None,
              test_data=None, test_labels=None, test_type_maps=None, test_files=None,
              test_labels_files=None, test_probs=None,
              channel_axis=None, load_files=True, batch_size=1, learning_rate=1e-5, SGD=False,
    ...
    out = _process_train_test(train_data=train_data, train_labels=train_labels, train_type_maps=train_type_maps,
                              train_files=train_files, train_labels_files=train_labels_files, train_probs=train_probs,
                              test_data=test_data, test_labels=test_labels, test_type_maps=test_type_maps,
                              test_files=test_files, test_labels_files=test_labels_files, test_probs=test_probs,
