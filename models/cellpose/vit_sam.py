...
class Transformer(nn.Module):
    ...
    def load_model(self, PATH, device, strict=False):
        ...
        if keys[0][:7] == "module.":
            ...
            # nout = 3
            # nclasses = 8
            # w0 = self.out.weight.data.detach().clone()
            # b0 = self.out.bias.data.detach().clone()
            # self.out = nn.Conv2d(256, (nout + nclasses) * ps ** 2, kernel_size=1).to(device)
            # # set weights for background map
            # i = 0
            # self.out.weight.data[i * ps ** 2: (i + 1) * ps ** 2] = -0.5 * w0[(nout - 1) * ps ** 2: nout * ps ** 2]
            # self.out.bias.data[i * ps ** 2: (i + 1) * ps ** 2] = b0[(nout - 1) * ps ** 2: nout * ps ** 2]
            # # set weights for maps to 4 nuclei classes
            # for i in range(1, nclasses):
            #     self.out.weight.data[i * ps ** 2: (i + 1) * ps ** 2] = 0.5 * w0[(nout - 1) * ps ** 2: nout * ps ** 2]
            #     self.out.bias.data[i * ps ** 2: (i + 1) * ps ** 2] = b0[(nout - 1) * ps ** 2: nout * ps ** 2]
            # self.out.weight.data[-(nout * ps ** 2):] = w0
            # self.out.bias.data[-(nout * ps ** 2):] = b0
