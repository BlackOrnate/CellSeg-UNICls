...
def load_images_labels(tdir, mask_filter="_masks", image_filter=None,
    ...
    image_dir = f"{tdir}/images/"
    mask_dir = f"{tdir}/new_labels/"
    image_ext = ".png"
    mask_ext = "_instance.npy"
    type_ext = "_classification.npy"
    ...
    for img_name in image_names:
        ...
        type_path = os.path.join(mask_dir, base + type_ext)
    ...
    print(f"{len(images)} images with masks loaded")
    return images, labels, type_maps, valid_names
...
def load_train_test_data(train_dir, test_dir=None, image_filter=None,
    ...
    """
    images, labels, type_maps, image_names = load_images_labels(train_dir, mask_filter,
                                                                image_filter, look_one_level_down)
    # testing data
    test_images, test_labels, test_type_maps, test_image_names = None, None, None, None
    if test_dir is not None:
        test_images, test_labels, test_type_maps, test_image_names = load_images_labels(
            test_dir, mask_filter, image_filter, look_one_level_down)

    return images, labels, type_maps, image_names, test_images, test_labels, test_type_maps, test_image_names


def load_only_test_data(test_dir=None):
    """
    Loads training and testing data for a Cellpose model.

    Args:
        test_dir (str): The directory path containing the testing data. Defaults to None.
    Returns:
        images, image_names,
    ...
    for patient_type in os.listdir(image_dir):
        for patient_id in os.listdir(os.path.join(image_dir, patient_type)):
            for region_name in os.listdir(os.path.join(image_dir, patient_type, patient_id)):
                for img_name in os.listdir(os.path.join(image_dir, patient_type, patient_id, region_name)):
                    img_path = os.path.join(image_dir, patient_type, patient_id, region_name, img_name)
                    image = np.array(Image.open(img_path)).astype(np.uint8)

                    images.append(image)
                    valid_names.append(img_name.split(".png")[0])
    print(f"{len(images)} images loaded")

    return images, valid_names
