...
def labels_to_flows(labels, type_maps=None, files=None, device=None, redo_flows=False, niter=None,
                    return_flows=True):
    ...
    if type_maps[0].ndim < 3:
        type_maps = [type_maps[n][np.newaxis, :, :] for n in range(nimg)]
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows:
        ...
        for n in iterator(nimg):
            ...
            flow = np.concatenate((labels[n], type_maps[n], labels[n] > 0.5, vecn), axis=0).astype(np.float32)
